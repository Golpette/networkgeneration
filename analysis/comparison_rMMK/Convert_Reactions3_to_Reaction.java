...
public class Convert_Reactions3_to_Reaction{
    ...
    // Convert the "equation" String in old file format (Reactions3) 
    // to new file format (Reaction)
    public static String convertRxnString( String rxn_string ){

	String output = "";
	output = rxn_string.replaceAll("H2O","");
	output = output.replaceAll("CO2\\(aq\\)","-co2");     //  "(" IS A METACHARACTER... Need "\\" to escape it
    ...
    // MichaelisMenten eqn solver needs list of "Reaction". This method creates "Reaction"
    //    object from a "Reactions3" object.
	...
	//   This only works since we have uni-uni reactions  
	// #################### CAREFUL: original network was uni-uni but when Bartek extended it to the
	// ####################   new format he included cleavage reactions.
	...
	while(true){
	    try{
		ss = scan2.next();
		if( ss.contains("C") && ss.contains("H") ){    
// ###############    ONLY WORKS BECAUSE all of our internal metabs contain at least 1 C and 1 H   #################
		    if( !subsFound ){
			reaxn = reaxn + " " + sub;
			subsFound = true;
		    }
